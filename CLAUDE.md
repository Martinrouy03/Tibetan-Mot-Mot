...
**Ne jamais modifier les groupes phonétiques fournis sans validation.** Les groupes définis par `/` dans les phonétiques sont la référence pour le découpage `words`. Si les groupes ne permettent pas de faire une correspondance claire avec la traduction, signaler le problème et demander comment regrouper. **Exception : si un groupe coupe une frontière grammaticale tibétaine évidente** (ex : particule ablative ལས་ collée à la syllabe suivante alors qu'elle clôt la proposition précédente), proposer un regroupement plus pertinent avec explication avant d'insérer — l'utilisateur valide ou corrige.
