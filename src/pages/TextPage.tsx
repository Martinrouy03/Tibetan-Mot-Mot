...
export default function TextPage() {
  ...
  const breakdownTranslationAbove = useAppSelector(
    (state) => state.ui.breakdownTranslationAbove,
  );
  ...
  const audioPlayerVisible = useAppSelector(
    (state) => state.ui.audioPlayerVisible,
  );
  ...
  const [scrollingMantraId, setScrollingMantraId] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    return () => {
      dispatch(setCurrentAudioSrc(null));
    };
  ...
  useEffect(() => {
    ...
    const rafId = requestAnimationFrame(() => {
      ...
      if (!spans[0]) return;
      const spanWidth =
        spans[0].getBoundingClientRect().width || wrapper.scrollWidth / 2;
      if (!spanWidth) return;
  ...
  const setPhraseRef = useCallback(
    (phraseId: string, el: HTMLDivElement | null) => {
      if (el) {
  ...
  useEffect(() => {
    ...
    if (!selectedPhraseId && allPhraseIds.length > 0) {
      const startId =
        lastClickedId.current && allPhraseIds.includes(lastClickedId.current)
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      if (Math.abs(wheelAccum.current) >= WHEEL_TICKS_PER_PHRASE) {
        ...
        const nextIndex = Math.max(
          0,
          Math.min(allPhraseIds.length - 1, currentIndex + step),
        );
    ...
    const handleScroll = () => {
      ...
      for (const [id, el] of phraseRefs.current) {
        ...
        const distance = Math.abs(
          rect.top + Math.min(rect.height / 2, 40) - viewportCenter,
        );
  ...
  const handlePhraseClick = (phraseId: string) => {
    ...
    dispatch(
      setSelectedPhrase(selectedPhraseId === phraseId ? null : phraseId),
    );
  };
  ...
  const handleScrollNext = useCallback(() => {
    ...
    const hommageEls = Array.from(
      document.querySelectorAll<HTMLElement>(".ta-hommage-pair"),
    );
    const currentHommageIdx = hommageEls.findIndex(
      (el) => el.getBoundingClientRect().top >= -4,
    );
    const inHommage =
      currentHommageIdx >= 0 &&
    ...
    const miphamEl = document.querySelector<HTMLElement>(
      '[data-phrase-id="ch-lg-42"]',
    );
    const miphamIdx = miphamEl ? hommageEls.indexOf(miphamEl) : -1;
    if (inHommage && miphamIdx >= 0 && currentHommageIdx <= miphamIdx) {
      const next =
        hommageEls[currentHommageIdx + 1] ?? hommageEls[hommageEls.length - 1];
      window.scrollBy({
    ...
    const phrases = Array.from(
      document.querySelectorAll<HTMLElement>(
        ".phrase-container, .tibetan-pair-row",
      ),
    );
    phrases.sort(
    ...
    if (bottomSection && bottomSection !== topSection) {
      window.scrollBy({
        top: bottomSection.getBoundingClientRect().top,
        behavior: "smooth",
      });
    } else {
      window.scrollBy({
        top: bottomPhrase.getBoundingClientRect().top,
        behavior: "smooth",
      });
    }
  }, []);
  ...
  return (
    <div className={hasSidebar ? "text-page-layout" : ""}>
      <nav className="mobile-strip">
        ...
        {hasSidebar && (
          <button
            className={`mobile-strip-btn${sidebarOpen ? " active" : ""}`}
            onClick={() => {
              setSidebarOpen((o) => !o);
        ...
        {currentAudioSrc && (
          <button
            className={`mobile-strip-btn${audioPlayerVisible ? " active" : ""}`}
        ...
        <button
          className={`mobile-strip-btn${mobileSettingsOpen ? " active" : ""}`}
          onClick={() => {
        ...
        {sidebarOpen && hasSidebar && (
          <>
            <div
              className="mobile-strip-overlay"
              ...
              <ul className="sidebar-nav-list">
                {navSections.map((s) => (
                  <li key={s.id}>
                    ...
                    >
                      {s.title}
                      {s.audioTimestamp !== undefined && (
                        <span className="sidebar-audio-icon">♪</span>
                      )}
                    </button>
                  </li>
                ))}
          ...
          <>
            ...
            <div className="mobile-strip-panel">
              <div className="settings-row settings-row-inline">
                ...
                <div className="font-size-control">
                  <button
                    ...
                    onClick={() => dispatch(changeFontSize(-3))}
                  >
                    −
                  </button>
              ...
              <div className="settings-row settings-row-inline">
                ...
                <button
                  className={`toggle-switch${!lightMode ? " on" : ""}`}
                ...
                <>
                  ...
                  <div className="settings-row settings-row-inline">
                    ...
                    <div className="radio-group">
                      <label
                        className={`radio-label ${!breakdownTranslationAbove ? "active" : ""}`}
                      >
                        <input
                          type="radio"
                          name="mobileBreakdownPos"
                          checked={!breakdownTranslationAbove}
                          onChange={() =>
                            breakdownTranslationAbove &&
                            dispatch(toggleBreakdownPosition())
                          }
                        />
                        ↓
                      </label>
                  ...
                  <div className="settings-row settings-row-inline">
                    ...
                    <div className="radio-group">
                      <label
                        className={`radio-label ${interactionMode === "click" ? "active" : ""}`}
                      >
                        <input
                          type="radio"
                          name="mobileInteractionMode"
                          value="click"
                          checked={interactionMode === "click"}
                          onChange={() =>
                            dispatch(
                              setInteractionMode("click" as InteractionMode),
                            )
                          }
                        />
                        Clic
                      ...
                      <label
                        className={`radio-label ${interactionMode === "fixed" ? "active" : ""}`}
                      >
                        <input
                          type="radio"
                          name="mobileInteractionMode"
                          ...
                          onChange={() =>
                            dispatch(
                              setInteractionMode("fixed" as InteractionMode),
                            )
                          }
                        />
                        Fixe
      ...
      {hasSidebar && (
        <nav
          className={`text-sidebar ${sidebarOpen ? "text-sidebar-open" : ""}`}
        >
      ...
      <div
        className={`text-page text-${text.id} ${interactionMode === "scroll" ? "text-page-scroll" : ""}`}
      >
        <button className="back-button" onClick={() => navigate("/")}>
          ← Retour aux textes
        </button>
        <h2 className="text-page-title">
          <span className="text-page-title-tibetan tibetan">
            {text.tibetanTitle}
          </span>
        ...
        {text.tibetanOnly &&
          text.sections.map((section) => {
            const regularPhrases = section.phrases.filter(
              (p) => p.type !== "nav-btn",
            );
            const navPhrases = section.phrases.filter(
              (p) => p.type === "nav-btn",
            );
            const rows: {
              left: (typeof regularPhrases)[0];
              right?: (typeof regularPhrases)[0];
            }[] = [];
            for (let i = 0; i < regularPhrases.length; i += 2) {
              rows.push({
                left: regularPhrases[i],
                right: regularPhrases[i + 1],
              });
            }
            const makeNavClick = (p: (typeof navPhrases)[0]) => () => {
              ...
              const navState: { from: string; seekTo?: number } = {
                from: text.id,
              };
              if (p.audioTimestamp !== undefined)
                navState.seekTo = p.audioTimestamp;
              if (hash) {
            ...
            return (
              <div key={section.id} className="section">
                  ...
                  (navPhrases.every((p) => p.navBack) ? (
                    <>
                      <div className="phrase-container phrase-no-interact nav-btn-container nav-btn-container-left">
                        <button
                          className="nav-btn"
                          onClick={makeNavClick(navPhrases[0])}
                        >{`← ${navPhrases[0].translation}`}</button>
                      </div>
                      <div className="phrase-container phrase-no-interact nav-btn-container nav-btn-container-left">
                        <button
                          className="nav-btn"
                          onClick={makeNavClick(navPhrases[1])}
                        >{`← ${navPhrases[1].translation}`}</button>
                      </div>
                    </>
                  ) : (
                    <div className="nav-btn-row">
                      <button
                        ...
                        onClick={makeNavClick(navPhrases[0])}
                      >
                        {navPhrases[0].navBack
                          ? `← ${navPhrases[0].translation}`
                          : `${navPhrases[0].translation} →`}
                      </button>
                      <button
                        className="nav-btn"
                        onClick={makeNavClick(navPhrases[1])}
                      >
                        {navPhrases[1].navBack
                          ? `← ${navPhrases[1].translation}`
                          : `${navPhrases[1].translation} →`}
                      </button>
                    </div>
                  ))}
                {navPhrases.length === 1 && (
                  <div
                    className={`nav-btn-container${navPhrases[0].navBack ? " nav-btn-container-left" : ""}`}
                  >
                    <button
                      className="nav-btn"
                      onClick={makeNavClick(navPhrases[0])}
        ...
        {!text.tibetanOnly &&
          text.sections.map((section) => {
            ...
            type PhrasePair = {
              label?: (typeof section.phrases)[0];
              normal: (typeof section.phrases)[0];
              image?: (typeof section.phrases)[0];
            };
            ...
            if (isHommageSection) {
              ...
              while (i < section.phrases.length) {
                ...
                if (
                  p.type === "instructions" &&
                  !p.tibetan &&
                  section.phrases[i + 1]?.type === "normal"
                ) {
                  const label = p;
                  const normal = section.phrases[i + 1];
                  const image =
                    section.phrases[i + 2]?.type === "image"
                      ? section.phrases[i + 2]
                      : undefined;
                  ...
                  p.type === "normal" &&
                  section.phrases[i + 1]?.type === "image"
                ) {
            ...
            return (
              ...
              >
                    ...
                    <div className="section-title-row">
                      ...
                      {section.id === "ch-pema-karpo" && (
                        <button
                          ...
                          onClick={() => setPemaKarpoCollapsed((c) => !c)}
                        >
                          {pemaKarpoCollapsed ? "▶" : "▼"}
                        </button>
                      )}
                ...
                <div className="phrases">
                  {section.id === "ch-pema-karpo" && pemaKarpoCollapsed
                    ? null
                    : isHommageSection
                      ? pairs.map(({ label, normal, image }) => {
                          ...
                          const buddhaName =
                            image && section.id === "ta-hommage"
                              ...
                              : null;
                          const prefix = buddhaName
                            ? `[${normalCount}. ${buddhaName}] `
                            : undefined;
                          const isSelected =
                            interactionMode === "fixed" ||
                            selectedPhraseId === normal.id;
                          const isLastPrayer = !image;
                          return (
                            <div
                              key={normal.id}
                              data-phrase-id={normal.id}
                              className={`ta-hommage-pair ${isLastPrayer ? "ta-hommage-pair-solo" : ""}`}
                            >
                              {image && (
                              ...
                              <div className="ta-hommage-right">
                                {label && (
                                  <div className="phrase phrase-special ta-hommage-label">
                                    <span
                                      className="phrase-special-translation"
                                      dangerouslySetInnerHTML={{
                                        __html: label.translation,
                                      }}
                                    />
                                  </div>
                                )}
                                {normal.type === "image-row" ? (
                                      ...
                                      >
                                        {srcs.map((src, idx) => (
                                          <img
                                            ...
                                            src={src}
                                            alt=""
                                            className="phrase-image-row-item"
                                            style={
                                              single
                                                ? { flex: "none", width: "30%" }
                                                : undefined
                                  ...
                                  <div className="phrase phrase-special">
                                    <span className="phrase-text tibetan">
                                      {normal.tibetan}
                                    </span>
                                    {normal.translation && (
                                      <span
                                  ...
                                  >
                                    {isSelected ? (
                                      ...
                                      <div className="phrase">
                                        {prefix && (
                                          <span className="breakdown-prefix">
                                            {prefix}
                                          </span>
                                        )}
                                        <span
                                          className={`phrase-text ${displayMode === "tibetan" ? "tibetan" : "phrase-text-phonetics"}`}
                                        >
                                          {displayMode === "tibetan"
                                            ? normal.tibetan
                                            : normal.phonetics}
                                        ...
                                        {showTranslation &&
                                          normal.translation && (
                                            <span
                                              className="phrase-inline-translation"
                                              dangerouslySetInnerHTML={renderTranslation(
                                                normal.translation,
                                              )}
                                            />
                                          )}
                      ...
                      : section.id === "ch-supplique"
                        ? (() => {
                            const renderInstr = (
                              ...
                              <div
                                key={p.id}
                                className="phrase-container phrase-no-interact"
                              >
                                <div className="phrase phrase-special">
                                  <span className="phrase-text tibetan">
                                    {p.tibetan}
                            ...
                            const renderVerse = (
                              ...
                              return (
                                <div
                                  key={p.id}
                                  ref={(el) => setPhraseRef(p.id, el)}
                                  data-phrase-id={p.id}
                                  className={`phrase-container ${interactionMode === "fixed" ? "phrase-no-interact" : ""}`}
                                  onClick={() =>
                                    interactionMode !== "fixed" &&
                                    handlePhraseClick(p.id)
                                  }
                                >
                                  {isSelected ? (
                                    <PhraseBreakdown
                        ...
                        : section.phrases.map(
                            (phrase, idx, filteredPhrases) => {
                              const isNormal = phrase.type === "normal";
                              const isMantra = phrase.type === "mantra";
                              const isMantraMain =
                                phrase.type === "mantra-main";
                              const isSpecial =
                                phrase.type === "instructions" ||
                                phrase.type === "colophon";
                              const isNoTranslation =
                                phrase.type === "no-translation";
                                  ...
                                  >
                                    <button
                                      ...
                                      onClick={() =>
                                        document
                                          .querySelector(
                                            `[data-phrase-id="${phrase.targetId}"]`,
                                          )
                                          ?.scrollIntoView({
                                            behavior: "smooth",
                                            block: "center",
                              ...
                              if (phrase.type === "nav-btn") {
                                ...
                                if (nextPhrase?.type === "nav-btn") {
                                  return (
                                    <div
                                      key={phrase.id}
                                      className="phrase-container phrase-no-interact nav-btn-row"
                                    >
                                      <button
                                        className="nav-btn"
                                        onClick={makeNavClick(phrase)}
                                      >
                                        {phrase.navBack
                                          ? `← ${phrase.translation}`
                                          : `${phrase.translation} →`}
                                      ...
                                      <button
                                        className="nav-btn"
                                        onClick={makeNavClick(nextPhrase)}
                                      >
                                        {nextPhrase.navBack
                                ...
                                return (
                                  <div
                                    key={phrase.id}
                                    className={`phrase-container phrase-no-interact nav-btn-container${phrase.navBack ? " nav-btn-container-left" : ""}`}
                                  >
                                    <button
                                      className="nav-btn"
                                      onClick={makeNavClick(phrase)}
                                ...
                                <React.Fragment key={phrase.id}>
                                  <div
                                    ...
                                    onClick={() =>
                                      isNormal &&
                                      interactionMode !== "fixed" &&
                                      handlePhraseClick(phrase.id)
                                    }
                                  >
                                    {phrase.stanceNumber !== undefined && (
                                      <span className="stance-number">
                                        [S.{phrase.stanceNumber}]
                                      </span>
                                    )}
                                    {isImage ? (
                                      ...
                                      <div
                                        className={`phrase phrase-mantra${scrollingMantraId === phrase.id ? " phrase-mantra-scrolling-active" : ""}`}
                                      >
                                        {scrollingMantraId === phrase.id ? (
                                          <div className="phrase-mantra-scroll-outer">
                                            <span className="mantra-hung-bg tibetan">
                                              ཧཱུྃ
                                            </span>
                                            <div className="phrase-mantra-scroll-clip">
                                              <div
                                                className="phrase-mantra-scroll-wrapper"
                                                ref={mantraWrapperRef}
                                              >
                                                <span
                                                  className={`phrase-text ${displayMode === "tibetan" ? "tibetan" : "phrase-text-phonetics"}`}
                                                >
                                      ...
                                      <div
                                        className={`phrase phrase-special${["sj-dd-6", "sj-dd-8"].includes(phrase.id) ? " phrase-special-right" : ""}`}
                                      >
                                        <span className="phrase-text tibetan">
                                          {phrase.tibetan}
                                        </span>
                ...
                {(section.id === "refuge" ||
                  ...
                  <button
                    className="scroll-top-btn"
                    onClick={() => {
                      const firstNormal = section.phrases.find(
                        (p) => p.type === "normal",
                      );
                      if (!firstNormal) return;
