...
export default function Header() {
  ...
  return (
    <header className={`header${isTextPage ? ' header-text-page' : ''}`}>
      ...
      {!isTextPage && (
        <img src="/logo_dkl.png" alt="DKL" className="header-logo-dkl header-logo-dkl-center" />
      )}
          ...
          {settingsOpen && (
            <div className="settings-menu">
              ...
              <div className="settings-row settings-row-inline">
                ...
                <button className={`toggle-switch${displayMode === 'phonetics' ? ' on' : ''}`} onClick={() => handleModeChange(displayMode === 'phonetics' ? 'tibetan' : 'phonetics')} aria-label="Phonétique" />
              </div>
              </>
              )}
              {isTextPage && !isTibetanOnly && (
                <>
                  <div className="settings-row settings-row-inline">
